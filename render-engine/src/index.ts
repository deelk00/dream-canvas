...
import { CustomAttribute } from './enums/custom-attribute.enum';
...
import { RootObject } from './dream-objects/root-object.class';
import { Vector } from './math/vector.class';
import { RelativePosition } from './enums/relative-position.enum';
import { SuppressContextMenuRootComponent } from './root-components/suppress-context-menu-component';
    ...
    public render = () => {
        ...
        for (const info of Object.values(this.renderingContexts)) {
            ...
            const visibleElements = elements.filter(e => {
                const dims = e.getVisibleRectangle();
                console.log(dims.endVector);
                ...
                return info.offset.subtract(dims.endVector).is(RelativePosition.AboveAndLeftFrom, startPos)
                    && canvasEndPos.is(RelativePosition.BelowAndRightFrom, canvasEndPos);
            });
